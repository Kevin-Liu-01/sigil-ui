...
Sigil puts every visual decision — colors, type, spacing, radius, motion, borders, shadows, 259 tokens total — into a single markdown file that an agent can read, reason about, and modify. Components don't own their appearance. The token spec does. Change the spec, everything updates.
