...
### 2.1 Built-in Presets (46: 44 named + default + template)
...
### 3.1 Package Components (350+ components across 258 files)
...
Re-exports from Radix UI and Base UI with `*Primitive` naming:
...
- Preset system (46 presets: 44 named + default + template, 7 categories, creation steps)
...
### 6.3 Design Philosophy: Constraints Over Context

Other agent-facing tools give agents reference material — screenshots, pattern libraries, anti-slop guidelines. The agent reads the context, then creates every component from scratch. This is the "reference approach." It helps, but agents still produce generic output because context is information, not enforcement.

Sigil takes the opposite approach: **constraints**. Pre-built components that consume tokens. Presets that define the entire visual space. A CLI that validates the result. The agent doesn't create a button — it uses `<Button>`, which reads `var(--s-primary)`. The visual identity is baked into the architecture, not described in a markdown file the agent might ignore.

### 6.4 Auditors

| Auditor | Layer | What it validates |
|---------|-------|-------------------|
| `sigil doctor` | Project | Config, tokens, components, deps, CSS imports, preset validity |
| `sigil diff` | Tokens | CSS changes since last sync |
| Preset validation | Presets | All 519 fields, OKLCH validity, scale progressions, duration ordering |
| TypeScript `satisfies` | Types | Every preset covers every token at compile time |
| WCAG contrast audit | Color | Text/background token combos meet AA per preset |
| Component conventions | Components | `forwardRef`, `className`, `var(--s-*)` only, `sigil-` prefix |
| Design system rules | Agent | `.cursor/rules/sigil-design-system.mdc` enforces token-only styling |
...
### 11.1 Documentation (`apps/web/docs`)
