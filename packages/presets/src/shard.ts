import type { SigilPreset } from "@sigil-ui/tokens";
import { completePreset } from "./_template";

export const shardPreset: SigilPreset = completePreset({
  name: "shard",
  metadata: {
    description: "Broken glass — angular, fragmented, aggressive geometry",
    author: "sigil-ui",
    version: "0.1.0",
    tags: ["aggressive", "angular", "fragmented", "sharp"],
    mood: "angular, aggressive, fragmented",
    inspiration: "Shattered glass, deconstructivism, glitch art",
  },
  tokens: {
    colors: {
      background: { light: "oklch(0.9851 0 0)", dark: "oklch(0.1448 0 0)" },
      surface: { light: "oklch(0.9674 0.0013 286.3748)", dark: "oklch(0.1913 0 0)" },
      "surface-elevated": { light: "oklch(1 0 0)", dark: "oklch(0.2264 0 0)" },
      "surface-sunken": { light: "oklch(0.9197 0.004 286.3201)", dark: "oklch(0.1149 0 0)" },
      text: { light: "oklch(0.1408 0.0044 285.8229)", dark: "oklch(0.9851 0 0)" },
      "text-secondary": { light: "oklch(0.2739 0.0055 286.0326)", dark: "oklch(0.8711 0.0055 286.2859)" },
      "text-muted": { light: "oklch(0.4419 0.0146 285.7864)", dark: "oklch(0.7118 0.0129 286.0665)" },
      "text-subtle": { light: "oklch(0.5517 0.0138 285.9384)", dark: "oklch(0.6363 0.0133 286.0153)" },
      "text-disabled": { light: "oklch(0.7118 0.0129 286.0665)", dark: "oklch(0.3703 0.0119 285.8054)" },
      "text-inverse": { light: "oklch(0.9851 0 0)", dark: "oklch(0.1408 0.0044 285.8229)" },
      border: { light: "oklch(0.651 0.0044 286.2768)", dark: "oklch(0.325 0.006 286)" },
      "border-muted": { light: "oklch(0.8024 0.0042 286.3052)", dark: "oklch(0.265 0.003 286)" },
      "border-strong": { light: "oklch(0.5517 0.0138 285.9384)", dark: "oklch(0.405 0.016 286)" },
      "border-interactive": { light: "oklch(0.5413 0.2466 293.009)", dark: "oklch(0.709 0.1592 293.5412)" },
      primary: "oklch(0.5413 0.2466 293.009)",
      "primary-hover": "oklch(0.4907 0.2412 292.5809)",
      "primary-muted": "oklch(0.9433 0.0284 294.5878)",
      "primary-contrast": "oklch(1 0 0)",
      secondary: "oklch(0.2103 0.0059 285.8851)",
      "secondary-hover": "oklch(0.1408 0.0044 285.8229)",
      "secondary-muted": "oklch(0.9674 0.0013 286.3748)",
      accent: "oklch(0.6559 0.2118 354.3084)",
      "accent-hover": "oklch(0.5916 0.218 0.5844)",
      "accent-muted": "oklch(0.9482 0.0276 342.2584)",
      success: "oklch(0.6271 0.1699 149.2138)",
      "success-muted": "oklch(0.9624 0.0434 156.7426)",
      warning: "oklch(0.6658 0.1574 58.3183)",
      "warning-muted": "oklch(0.9619 0.058 95.6174)",
      error: "oklch(0.5771 0.2152 27.325)",
      "error-muted": "oklch(0.9356 0.0309 17.7171)",
      info: "oklch(0.5461 0.2152 262.8809)",
      "info-muted": "oklch(0.9319 0.0316 255.5855)",
      "gradient-start": "oklch(0.5413 0.2466 293.009)",
      "gradient-end": "oklch(0.6559 0.2118 354.3084)",
      glow: "oklch(0.5413 0.2466 293.009 / 0.25)",
      highlight: { light: "oklch(0.9433 0.0284 294.5878)", dark: "oklch(0.6056 0.2189 292.7172 / 0.15)" },
    },
    typography: {
      "font-display": "'PP Fragment Sans', system-ui, sans-serif",
      "font-body": "'PP Fraktion Sans', system-ui, sans-serif",
      "font-mono": "'PP Fraktion Mono', ui-monospace, monospace",
      "size-xs": "0.75rem",
      "size-sm": "0.8125rem",
      "size-base": "0.9375rem",
      "size-lg": "1.0625rem",
      "size-xl": "1.25rem",
      "size-2xl": "1.5rem",
      "size-3xl": "1.875rem",
      "size-4xl": "2.25rem",
      "size-5xl": "3rem",
      "size-6xl": "3.75rem",
      "weight-normal": "400",
      "weight-medium": "500",
      "weight-semibold": "600",
      "weight-bold": "700",
      "weight-extrabold": "800",
      "leading-tight": "1.1",
      "leading-normal": "1.45",
      "leading-relaxed": "1.6",
      "leading-loose": "1.75",
      "tracking-tighter": "-0.04em",
      "tracking-tight": "-0.02em",
      "tracking-normal": "0em",
      "tracking-wide": "0.05em",
      "tracking-wider": "0.1em",
      "heading-weight": "400",
      "heading-tracking": "-0.01em",
      "heading-transform": "none",
      "heading-family": "'PP Fragment Sans', system-ui, sans-serif",
    },
    spacing: {
      scale: [2, 4, 8, 12, 16, 20, 28, 40, 56, 72],
      unit: "px",
      "button-px": "20px",
      "button-py": "10px",
      "button-px-sm": "14px",
      "button-py-sm": "6px",
      "button-px-lg": "28px",
      "button-py-lg": "14px",
      "card-padding": "14px",
      "card-padding-sm": "8px",
      "card-padding-lg": "20px",
      "input-px": "12px",
      "input-py": "8px",
      "badge-px": "8px",
      "badge-py": "2px",
      "section-py": "4.5rem",
      "section-py-lg": "6.75rem",
      "navbar-height": "54px",
      "navbar-px": "14px",
      "footer-py": "36px",
      "modal-padding": "20px",
      "popover-padding": "12px",
      "tooltip-padding": "6px 10px",
      "table-cell-px": "12px",
      "table-cell-py": "8px",
    },
    layout: {
      "content-max": "1200px",
      "content-max-narrow": "720px",
      "content-max-wide": "1440px",
      "page-margin": "16px",
      "page-margin-sm": "12px",
      "page-margin-lg": "32px",
      gutter: "12px",
      "gutter-sm": "6px",
      "gutter-lg": "20px",
      "grid-columns": "12",
      "grid-gap": "12px",
      "bento-gap": "6px",
      "bento-radius": "0px",
      "bento-min-height": "180px",
      "section-gap": "36px",
      "sidebar-width": "228px",
      "sidebar-collapsed": "52px",
      "footer-columns": "4",
      "stack-gap": "12px",
      "stack-gap-sm": "6px",
      "stack-gap-lg": "20px",
      "prose-max": "62ch",
    },
    sigil: {
      "grid-cell": "36px",
      "cross-arm": "10px",
      "cross-stroke": "2px",
      "rail-gap": "12px",
      "card-radius": "0px",
      "gutter-pattern": "diamond",
      "margin-pattern": "zigzag",
      "band-height": "36px",
      "divider-thickness-sm": "calc(var(--s-grid-cell) / 2)",
      "divider-thickness-md": "var(--s-grid-cell)",
      "divider-thickness-lg": "calc(2 * var(--s-grid-cell))",
      "divider-thickness-xl": "calc(3 * var(--s-grid-cell))",
    },
    radius: {
      none: "0px",
      sm: "0px",
      md: "0px",
      lg: "0px",
      xl: "0px",
      "2xl": "0px",
      full: "0px",
      button: "0px",
      card: "0px",
      input: "0px",
      badge: "0px",
      modal: "0px",
      popover: "0px",
      tooltip: "0px",
      image: "0px",
      bento: "0px",
    },
    shadows: {
      sm: "0 1px 2px rgba(0, 0, 0, 0.05)",
      md: "0 2px 4px rgba(0, 0, 0, 0.08)",
      lg: "0 4px 8px rgba(0, 0, 0, 0.1)",
      xl: "0 8px 16px rgba(0, 0, 0, 0.12)",
      "2xl": "0 16px 32px rgba(0, 0, 0, 0.15)",
      glow: "0 0 20px rgba(124, 58, 237, 0.3)",
      "glow-sm": "0 0 10px rgba(124, 58, 237, 0.2)",
      colored: "0 4px 12px rgba(124, 58, 237, 0.15)",
      inner: "inset 0 2px 4px rgba(0, 0, 0, 0.08)",
      card: "none",
      button: "none",
      "button-hover": "0 0 12px rgba(124, 58, 237, 0.2)",
      dropdown: "0 4px 12px rgba(0, 0, 0, 0.12)",
      modal: "0 16px 48px rgba(0, 0, 0, 0.2)",
    },
    motion: {
      duration: {
        instant: "0ms",
        fast: "80ms",
        normal: "150ms",
        slow: "250ms",
        slower: "380ms",
        slowest: "500ms",
      },
      easing: {
        default: "cubic-bezier(0.22, 0.68, 0, 1)",
        in: "cubic-bezier(0.6, 0, 1, 0.6)",
        out: "cubic-bezier(0, 0.4, 0.4, 1)",
        "in-out": "cubic-bezier(0.6, 0, 0.4, 1)",
        spring: "cubic-bezier(0.22, 1.4, 0.36, 1)",
        bounce: "cubic-bezier(0.22, 1.6, 0.36, 1)",
      },
      "enter-preset": "150ms cubic-bezier(0.22, 0.68, 0, 1)",
      "exit-preset": "100ms cubic-bezier(0.6, 0, 1, 0.6)",
      "hover-scale": "1.02",
      "press-scale": "0.96",
      "hover-lift": "-2px",
      "stagger-interval": "35ms",
      "stagger-interval-fast": "15ms",
    },
    borders: {
      width: { none: "0px", thin: "1px", medium: "1.5px", thick: "2px" },
      style: "solid",
      "card-border": "1px solid var(--s-border)",
      "card-border-hover": "1px solid var(--s-primary)",
      "button-border": "0px solid transparent",
      "input-border": "1px solid var(--s-border)",
      "divider-style": "solid",
      "divider-width": "1px",
    },
    buttons: {
      "font-weight": "800",
      "text-transform": "uppercase",
      "letter-spacing": "0.08em",
      "font-family": "'PP Fraktion Sans', system-ui, sans-serif",
      "border-width": "0px",
      "hover-effect": "fill",
      "active-scale": "0.96",
      "icon-gap": "8px",
      "min-width": "0px",
    },
    cards: {
      "border-style": "solid",
      "border-width": "1.5px",
      "hover-effect": "border",
      "hover-border-color": "var(--s-primary)",
      padding: "16px",
      "header-padding": "16px 16px 0",
      "footer-padding": "0 16px 16px",
      "title-size": "var(--s-size-lg)",
      "title-weight": "700",
      "description-size": "var(--s-size-sm)",
      background: "surface",
      shadow: "none",
      outline: true,
      "outline-on-hover-only": false,
      "gap-between": "12px",
      "content-padding-x": "16px",
      "content-padding-y": "14px",
    },
    headings: {
      "h1-size": "3rem",
      "h1-weight": "800",
      "h1-tracking": "-0.04em",
      "h1-leading": "1.05",
      "h2-size": "2.25rem",
      "h2-weight": "800",
      "h2-tracking": "-0.03em",
      "h2-leading": "1.1",
      "h3-size": "1.5rem",
      "h3-weight": "700",
      "h3-tracking": "0em",
      "h3-leading": "1.15",
      "h4-size": "1.125rem",
      "h4-weight": "700",
      "h4-tracking": "0em",
      "h4-leading": "1.2",
      "display-size": "4rem",
      "display-weight": "800",
      "display-tracking": "-0.05em",
      "display-leading": "0.95",
    },
    navigation: {
      "navbar-height": "54px",
      "navbar-backdrop-blur": "8px",
      "navbar-border": "1px solid var(--s-border-muted)",
      "navbar-bg-opacity": "0.92",
      "navbar-padding-x": "24px",
      "navbar-position": "sticky",
      "navbar-logo-height": "26px",
      "navbar-logo-gap": "10px",
      "navbar-item-gap": "24px",
      "navbar-item-padding-x": "14px",
      "navbar-item-padding-y": "6px",
      "navbar-item-size": "0.875rem",
      "navbar-shadow": "none",
      "navbar-height-scrolled": "48px",
      "navbar-mobile-breakpoint": "768px",
      "nav-link-weight": "600",
      "nav-link-size": "var(--s-size-sm)",
      "nav-link-hover": "color",
      "breadcrumb-separator": "//",
      "pagination-radius": "0px",
      "sidebar-width": "228px",
      "sidebar-item-radius": "0px",
      "sidebar-item-padding": "6px 10px",
    },
    backgrounds: {
      pattern: "diamond",
      "pattern-opacity": "0.05",
      "pattern-scale": "8px",
      noise: false,
      "noise-opacity": "0",
      "gradient-angle": "135deg",
      "gradient-type": "linear",
      "hero-pattern": "grid",
      "section-divider": "diagonal",
    },
    code: {
      "font-family": "'PP Fraktion Mono', ui-monospace, monospace",
      "font-size": "0.8125rem",
      "line-height": "1.55",
      bg: "#f4f4f5",
      border: "1px solid #d4d4d8",
      "border-radius": "0px",
      padding: "16px 20px",
      "tab-size": "2",
      "selection-bg": "rgba(124, 58, 237, 0.15)",
      "comment-color": "#71717a",
      "keyword-color": "#7c3aed",
      "string-color": "#16a34a",
      "number-color": "#ec4899",
      "function-color": "#2563eb",
    },
    inputs: {
      height: "38px",
      "height-sm": "30px",
      "height-lg": "46px",
      "border-width": "1px",
      "focus-ring-width": "2px",
      "focus-ring-color": "var(--s-primary)",
      "focus-ring-offset": "0px",
      "placeholder-color": "var(--s-text-muted)",
      "error-border-color": "var(--s-error)",
      "label-size": "var(--s-size-xs)",
      "label-weight": "700",
      "label-spacing": "4px",
      "helper-size": "var(--s-size-xs)",
    },
    alignment: {
      "title-align": "left",
    },
    sections: {
      "rail-margin": "16px",
      "padding-y": "4.5rem",
      "padding-y-hero": "6.75rem",
      "padding-y-sm": "2.25rem",
      gap: "20px",
      "padding-y-lg": "6.75rem",
      "padding-y-xl": "9rem",
      "title-align": "center",
      "heading-size": "clamp(1.5rem, 4vw, 2.5rem)",
      "heading-align": "center",
      "heading-max-width": "40rem",
      "heading-margin-bottom": "16px",
      "description-max-width": "32rem",
      "description-gap": "12px",
      "content-gap": "24px",
      "grid-columns": "3",
      "grid-gap": "24px",
      "alternate-bg": "var(--s-surface)",
      "alternate-bg-mode": "surface",
    },
    dividers: {
      style: "solid",
      width: "2px",
      "full-bleed": true,
      spacing: "0px",
      opacity: "0.8",
      "max-width": "100%",
      "margin-y": "0px",
      thickness: "1px",
      "gradient-start": "var(--s-primary)",
      "gradient-end": "var(--s-secondary)",
    },
    gridVisuals: {
      "line-color": "var(--s-border-muted)",
      "cell-border": true,
      "show-lines": true,
      "line-width": "1.5px",
    },
    cursor: {
      variant: "ring",
      size: "24px",
      "dot-size": "4px",
      "stroke-width": "1.5px",
      "tick-size": "6px",
      gap: "3px",
      radius: "0px",
      color: "var(--s-text)",
      "ring-color": "var(--s-primary)",
      "dot-color": "var(--s-primary)",
      glow: "none",
      opacity: "1",
      "blend-mode": "normal",
      "z-index": "9999",
      "hide-native": false,
    },
    scrollbar: {
      width: "8px",
      height: "8px",
      padding: "2px",
      radius: "9999px",
      track: "transparent",
      thumb: "var(--s-border-muted)",
      "thumb-hover": "var(--s-border)",
      "thumb-active": "var(--s-border-strong)",
      corner: "transparent",
      border: "none",
      "firefox-width": "thin",
      gutter: "stable",
      visibility: "auto",
    },
    focus: {
      "ring-width": "2px",
      "ring-color": "var(--s-primary)",
      "ring-offset": "2px",
      "ring-shadow": "0 0 0 2px var(--s-primary)",
      "outline-color": "var(--s-primary)",
    },
    overlays: {
      bg: "rgb(0 0 0 / 0.5)",
      blur: "8px",
      surface: "var(--s-surface-elevated)",
      border: "1px solid var(--s-border-muted)",
      shadow: "var(--s-shadow-xl)",
      radius: "var(--s-radius-lg)",
      padding: "20px",
      "z-index": "50",
    },
    dataViz: {
      "series-1": "var(--s-primary)",
      "series-2": "var(--s-secondary)",
      "series-3": "var(--s-accent)",
      "series-4": "var(--s-info)",
      "series-5": "var(--s-success)",
      positive: "var(--s-success)",
      negative: "var(--s-error)",
      neutral: "var(--s-text-muted)",
      grid: "var(--s-border-muted)",
      axis: "var(--s-border)",
      label: "var(--s-text-secondary)",
      "tooltip-bg": "var(--s-surface-elevated)",
      "tooltip-border": "var(--s-border-muted)",
    },
    media: {
      radius: "var(--s-radius-md)",
      border: "none",
      outline: "1px solid rgb(0 0 0 / 0.08)",
      shadow: "none",
      bg: "var(--s-surface-sunken)",
      "object-fit": "cover",
    },
    controls: {
      height: "38px",
      "height-sm": "30px",
      "height-lg": "46px",
      "hit-area": "44px",
      "icon-size": "16px",
      "handle-size": "20px",
      "track-height": "4px",
      "track-bg": "var(--s-border-muted)",
      "track-fill": "var(--s-primary)",
      "thumb-bg": "var(--s-surface-elevated)",
      "thumb-border": "2px solid var(--s-primary)",
    },
    componentSurfaces: {
      bg: "var(--s-surface)",
      "bg-elevated": "var(--s-surface-elevated)",
      "bg-muted": "var(--s-surface-sunken)",
      border: "var(--s-border)",
      "border-muted": "var(--s-border-muted)",
      "border-strong": "var(--s-border-strong)",
      text: "var(--s-text)",
      "text-muted": "var(--s-text-muted)",
      contrast: "var(--s-primary-contrast)",
      "hover-bg": "var(--s-surface-elevated)",
      "active-bg": "var(--s-surface-sunken)",
      "selected-bg": "var(--s-primary-muted)",
    },
    hero: {
      "min-height": "auto",
      "padding-y": "90px",
      "padding-y-sm": "72px",
      "padding-x": "var(--s-page-margin, 24px)",
      "content-max": "640px",
      "content-basis": "min(42%, var(--s-hero-content-max, 640px))",
      "content-align": "center",
      layout: "centered",
      "media-position": "below",
      "media-width": "50%",
      "media-radius": "var(--s-radius-xl)",
      "split-gap": "var(--s-section-subsection-gap, var(--s-grid-cell))",
      "title-size": "clamp(2.25rem, 6vw, 4.5rem)",
      "title-max-width": "56rem",
      "description-size": "1.125rem",
      "description-max-width": "32rem",
      "description-gap": "24px",
      "actions-gap": "14px",
      "actions-margin-top": "32px",
      "action-padding-x": "28px",
      "action-padding-y": "12px",
      "badge-gap": "24px",
      "overlay-opacity": "0.4",
      "scroll-indicator": false,
      "gradient-overlay": "none",
    },
    cta: {
      "padding-y": "72px",
      "padding-x": "var(--s-page-margin, 24px)",
      "max-width": "var(--s-content-max, 1200px)",
      layout: "centered",
      border: true,
      "border-radius": "var(--s-radius-xl)",
      bg: "var(--s-surface)",
      "title-size": "clamp(1.5rem, 4vw, 2.5rem)",
      "description-max-width": "32rem",
      "description-gap": "12px",
      "actions-gap": "14px",
      "actions-margin-top": "32px",
      "action-padding-x": "28px",
      "action-padding-y": "12px",
      "split-gap": "54px",
    },
    footer: {
      "padding-y": "54px",
      "border-top": "1px solid var(--s-border)",
      columns: "4",
      "column-gap": "32px",
      "row-gap": "28px",
      "logo-height": "24px",
      "tagline-max-width": "20rem",
      "link-size": "0.875rem",
      "link-gap": "8px",
      "group-title-size": "0.875rem",
      "group-title-weight": "600",
      "social-icon-size": "20px",
      "social-gap": "12px",
      "bottom-bar-padding": "18px 0",
      "bottom-bar-border": "1px solid var(--s-border-muted)",
    },
    banner: {
      height: "auto",
      "padding-y": "8px",
      "padding-x": "16px",
      "font-size": "0.875rem",
      "font-weight": "500",
      "icon-size": "16px",
      "icon-gap": "12px",
      "border-width": "1px",
      "border-position": "bottom",
      position: "top",
      "dismiss-size": "16px",
      radius: "0px",
    },
    pageRhythm: {
      mode: "locked",
      snap: true,
      "band-stroke": "visual",
      "hairline-width": "1px",
      "hairline-spacing": "var(--s-section-gap, 0px)",
      "hairline-style": "solid",
      density: "normal",
      "section-gap": "0px",
      "section-gap-sm": "0px",
      "section-gap-lg": "0px",
      "first-section-offset": "0px",
      "last-section-margin": "0px",
      "alternate-bg": true,
      "alternate-bg-color": "var(--s-surface)",
      "divider-between": false,
      "max-content-width": "var(--s-content-max, 1200px)",
      "vertical-rhythm-unit": "9px",
      "scroll-snap": false,
      "responsive-scale": "0.9",
      "responsive-breakpoint": "768px",
    },

  },
});
