...
function generateCustomPresetFile(
  ...
  const primary = primaryColor || "oklch(0.65 0.15 280)";
  const primaryHover = primaryColor || "oklch(0.60 0.18 280)";
  const primaryMuted = primaryColor || "oklch(0.90 0.04 280)";
  const font = displayFont ? `"${displayFont}", system-ui, sans-serif` : "'PP Neue Montreal', system-ui, sans-serif";

  if (baseName !== "_blank") {
    return `import type { SigilPreset } from "@sigil-ui/tokens";
import { _templatePreset } from "@sigil-ui/presets/_template";
import { ${baseName}Preset } from "@sigil-ui/presets/${baseName}";
...
export const ${camel}Preset: SigilPreset = {
  name: "${name}",
  metadata: {
    description: "Custom preset — edit all tokens",
    author: "you",
    version: "0.1.0",
    tags: ["custom"],
    mood: "your mood here",
