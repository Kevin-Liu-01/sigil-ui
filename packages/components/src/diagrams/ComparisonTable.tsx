...
export const ComparisonTable = forwardRef<HTMLDivElement, ComparisonTableProps>(
  function ComparisonTable({ features, columns, className, ...rest }, ref) {
    return (
      <div ref={ref} data-slot="comparison-table" className={cn("w-full overflow-auto border border-[style:var(--s-border-style,solid)] border-[color:var(--s-border)] rounded-[var(--s-radius-md,6px)] shadow-[var(--s-shadow-sm)]", className)} {...rest}>
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {features.map((feature) => (
              <tr
                key={feature.name}
                className="border-b border-[color:var(--s-border)] hover:bg-[var(--s-surface-elevated)] transition-colors duration-[var(--s-duration-fast,150ms)]"
              >
