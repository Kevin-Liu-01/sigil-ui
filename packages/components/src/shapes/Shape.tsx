...
export interface ShapeProps extends Omit<SVGAttributes<SVGSVGElement>, "size"> {
  /** Shape type. @default "diamond" */
  variant?: ShapeVariant;
  /** Size — preset string or any positive pixel number. @default "md" */
...
function resolveSize(size: ShapeSize | number | undefined): number {
  if (typeof size === "number" && Number.isFinite(size) && size > 0) return size;
  if (typeof size === "string" && size in sizes) return sizes[size as ShapeSize];
  return sizes.md;
}
