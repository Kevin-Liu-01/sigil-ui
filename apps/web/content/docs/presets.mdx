---
title: Presets
description: "31 curated visual identities — switch your entire design system with one config change."
---
...
```bash
# List all 31 presets with categories and descriptions
npx sigil preset list
...
```bash
npx sigil compile ./presets/my-brand.ts --out ./public/my-brand.css
```
...
```bash
npx sigil diff --preset sigil --preset cobalt
```
