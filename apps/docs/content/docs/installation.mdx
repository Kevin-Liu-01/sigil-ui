...
```bash
# Start with a preset
npx sigil init --preset minimal
npx sigil init --preset brutalist

# Specify output directory
npx sigil init --cwd ./apps/web
```
...
Add the Sigil token stylesheet to your root layout or entry point. This injects all `--r-*` CSS custom properties.
...
```css title="app/globals.css"
:root {
  --r-primary: oklch(0.55 0.20 145);
  --r-card-radius: 16px;
}
```

Or create a full preset file and compile it with the CLI:
